...
export default function createMulheres(THREE, { envMap }) {
    ...
    xPositions.forEach((x, index) => {
        ...
        const dressMat = materials.dress(dressColors[index]);
        ...
        const neck = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.06, 0.1, 12), materials.skin);
        neck.position.y = 1.03;
        person.add(neck);

        const head = new THREE.Mesh(new THREE.SphereGeometry(0.15, 20, 16), materials.skin);
        head.position.y = 1.18;
        head.castShadow = true;
        person.add(head);

        const hair = new THREE.Mesh(new THREE.SphereGeometry(0.14, 16, 12), materials.hair);
        hair.position.y = 1.25;
        hair.scale.set(1, 0.8, 1);
        hair.castShadow = true;
        person.add(hair);

        const armL = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.34, 0.1), materials.skin);
        armL.position.set(-0.2, 0.82, 0);
        armL.rotation.z = 0.3;
        armL.castShadow = true;
        person.add(armL);

        const armR = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.34, 0.1), materials.skin);
    ...
    inner.add(fakeShadow(THREE, 1.7, 0.28));
